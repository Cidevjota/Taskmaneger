...
import {
  CrmCadencia, CrmCadenciaEtapa, CrmPrioridade,
  CRM_DIAS_SEMANA, CRM_PRIORIDADES, CRM_TIPOS_ACAO,
} from '../../lib/crmTypes';
          ...
          {cadencias.map(c => (
            ...
            >
              ...
              <p className="text-[10px] text-zinc-600 truncate mt-0.5">
                {etapas.filter(e => e.cadenciaId === c.id).length} etapas · {c.maxTentativas} tentativas
              </p>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="max-w-3xl space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
        ...
        <Campo label="Máx. tentativas">
          <input type="number" min={1} value={form.maxTentativas} disabled={!podeEditar}
            onChange={e => set({ maxTentativas: Math.max(1, Number(e.target.value) || 1) })} className={inputCls} />
        </Campo>
        <Campo label="Prioridade">
          <select value={form.prioridade} onChange={e => set({ prioridade: e.target.value as CrmPrioridade })}
            disabled={!podeEditar} className={selectCls}>
            {CRM_PRIORIDADES.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
          </select>
        </Campo>
        <Campo label="SLA (horas)">
          <input type="number" min={1} value={form.slaHoras} disabled={!podeEditar}
            onChange={e => set({ slaHoras: Math.max(1, Number(e.target.value) || 1) })} className={inputCls} />
        </Campo>
            ...
            {etapas.map((etapa, i) => (
              ...
              >
                ...
                <label className="flex items-center gap-1.5 text-[10px] text-zinc-500 shrink-0">
                  após
                  <input
                    type="number" min={0} defaultValue={etapa.intervaloHoras} disabled={!podeEditar}
                    onBlur={e => Number(e.target.value) !== etapa.intervaloHoras &&
                      onSalvarEtapa({ ...etapa, cadenciaId: etapa.cadenciaId, intervaloHoras: Math.max(0, Number(e.target.value) || 0) })}
